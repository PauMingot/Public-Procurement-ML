...
OUTPUT_DIR = RUTA / "salida_modelo_XGB_Importe"
OUTPUT_DIR.mkdir(exist_ok=True)

# --- RELLENA CON LO QUE TE SALGA DEL TUNEO ---
config_m1 = {
    "n_estimators": 209,       # Pon aquí el best_n_trees que te escupa el Tuneo
    "learning_rate": 0.2,     
    "max_depth": 4,          
    "reg_lambda": 1,
    "gamma": 0,
...
# ------------------------------------------------------------
# 4. MODELO 0: BASELINE ABSOLUTO
# ------------------------------------------------------------
print("\n" + "="*60)
print("--- MODELO 0: BASELINE ABSOLUTO ---")
...
try:
    ...
    # 🚨 EXTRAEMOS LOS EUROS REALES DIRECTAMENTE DEL TARGET
    y_val_euros_real_curva = val_curva[TARGET].values

    X_t = train_curva.drop(columns=[TARGET] + [c for c in DROP_COLS if c in train_curva.columns], errors='ignore')
    y_t_log = np.log1p(train_curva[TARGET]) # Entrenamos en Logaritmos
    
    X_v = val_curva.drop(columns=[TARGET] + [c for c in DROP_COLS if c in val_curva.columns], errors='ignore')
    # No necesitamos y_v_log porque evaluaremos directamente contra y_val_euros_real_curva
    ...
    modelo_curva = XGBRegressor(
        **{k: v for k, v in config_m1.items() if k != 'n_estimators'},
        n_estimators=550,
        random_state=42,
        n_jobs=-1
    )
    modelo_curva.fit(X_t_trans, y_t_log, verbose=False)
    ...
    for i in rango_arboles:
        # Predecimos en espacio Logarítmico
        pred_log = modelo_curva.predict(X_v_trans, iteration_range=(0, i))
        
        # Destransformamos a Euros mediante la exponencial
        pred_log_segura = np.clip(pred_log, -np.inf, 30)
        pred_euros = np.expm1(pred_log_segura)
    ...
    plt.savefig(OUTPUT_DIR / 'Curva_Early_Stopping_XGB_Importe.png', bbox_inches='tight', dpi=300)
    plt.close()
...
# ------------------------------------------------------------
# 5. MODELO 1: XGBOOST GANADOR
# ------------------------------------------------------------
print("\n" + "="*60)
print(f"--- MODELO 1: XGBoost Ganador ---")
...
y_pred_test_xgb_log = pipeline_xgb.predict(X_test)
metrics_xgb = evaluar_en_euros('XGBoost (Test)', y_pred_test_xgb_log)

joblib.dump(pipeline_xgb, OUTPUT_DIR / "xgb_ganador_importe.pkl")


# ------------------------------------------------------------
# 5.5 DIAGNÓSTICO DE OVERFITTING (XGBOOST - ESCALA REAL EUROS)
# ------------------------------------------------------------
print("\n" + "="*60)
print("--- DIAGNÓSTICO DE OVERFITTING (XGBOOST - EUROS) ---")

# 1. Recuperamos los euros reales de Train
y_train_euros_real = full_train_df[TARGET].values

# 2. Predecimos en Train con XGBoost (el modelo escupe logaritmos)
y_pred_train_log = pipeline_xgb.predict(X_train)
...
r2_train_xgb = r2_score(y_train_euros_real, y_pred_train_euros)
mae_train_xgb = mean_absolute_error(y_train_euros_real, y_pred_train_euros)

# 5. Imprimimos comparativa Train vs Test
print(f"R²  Train: {r2_train_xgb:.4f}  |  R²  Test: {metrics_xgb['R²']:.4f}")
print(f"MAE Train: {mae_train_xgb:,.2f} € |  MAE Test: {metrics_xgb['MAE (€)']:,.2f} €")

if mae_train_xgb > 0:
    diferencia_mae = ((metrics_xgb['MAE (€)'] - mae_train_xgb) / mae_train_xgb) * 100
...
try:
    ...
    plt.title("Importancia de las Variables en el Importe (XGBoost)")
    plt.xlabel("Importancia Relativa (Gain)")
    plt.ylabel("Variable")
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "importancia_variables_xgb_importe.png", dpi=300)
    plt.close()
...
try:
    y_pred_xgb_euros = np.expm1(np.clip(y_pred_test_xgb_log, -np.inf, 30))
    y_pred_xgb_euros[y_pred_xgb_euros < 0] = 0
    ...
    sns.boxplot(
        ...
        orient='h',
        showfliers=False,
        palette=['#4C72B0', '#D62728'] # Rojo para XGBoost
    )
    plt.title('Distribución del Error Porcentual Absoluto (APE)\nBaseline vs. XGBoost (Importe)', fontsize=13, fontweight='bold')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / '1_comparativa_error_boxplot_XGB.png', dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / '3_scatter_XGB_Zoom.png', dpi=300)
    plt.close()
...
try:
    ...
    plt.title("Impacto de Features en la Predicción (SHAP - XGBoost)")
    plt.savefig(OUTPUT_DIR / 'shap_summary_plot_XGB.png', bbox_inches='tight', dpi=300)
    plt.close()
    
except Exception as e:
    print(f"Error al generar SHAP: {e}")

print("\n🎉 ¡Script de XGBoost completado!")